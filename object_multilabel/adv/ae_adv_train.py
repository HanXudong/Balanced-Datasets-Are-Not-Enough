...
from ae_adv_model import ObjectMultiLabelAdv
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str,
            help='specific checkpoint to resume',
    ...
    checkpoint = torch.load('./origin_0/model_best.pth.tar')
    ...
    best_performance = 0
    if args.resume:
        if os.path.isfile(os.path.join(args.save_dir, args.checkpoint)):
            print("=> loading checkpoint '{}'".format(args.save_dir))
            checkpoint = torch.load(os.path.join(args.save_dir, args.checkpoint))
    ...
    test_balanced(args, 0, model, criterion, criterionL1, val_loader_gender_balanced, val_logger, logging=False)
...
def train(args, epoch, model, criterion, criterionL1, train_loader, optimizer, \
        train_logger, print_every=50000, logging=True):
    model.train()
    nProcessed = 0
    ...
    for batch_idx, (images, targets, genders, image_ids) in enumerate(t):
        ...
        task_pred, adv_pred, autoencoded_images = model(images) # if conv5, adv_pred.size is (batch_size, 49, 2)
        ...
        l1_loss_logger.update(l1_loss.item())
        ...
        if (batch_idx + 1) % print_every == 0:
            task_meanAP = average_precision_score(task_truth.numpy(), task_preds.numpy(), average = 'macro')
            print('Train: {0:3d}, task loss: {1:.3f}, adv loss: {2:.3f}, l1 loss: {3:.3f}, task meanAP: {4:.3f}' \
                    ', adv acc: {5:.3f}, n_process: {6}'.format(batch_idx + 1,  task_loss_logger.avg, adv_loss_logger.avg, \
                    l1_loss_logger.avg, task_meanAP*100, accuracy_score(adv_truth, adv_preds)*100, nProcessed))
...
def test_balanced(args, epoch, model, criterion, criterionL1, val_loader, \
        val_logger, print_every=10000, logging=True):
    model.eval()
    nProcessed = 0
    task_preds, adv_preds = [], []
    task_truth, adv_truth = [], []
    ...
    for batch_idx, (images, targets, genders, image_ids) in enumerate(t):
        ...
        loss = task_loss + adv_loss + l1_loss

        # Print log info
        nProcessed += len(images)
        if (batch_idx + 1) % print_every == 0:
            task_meanAP = average_precision_score(task_truth.numpy(), task_preds.numpy(), average = 'macro')
            print('Test: {0:3d}, task loss: {1:.3f}, adv loss: {2:.3f}, l1 loss: {3:.3f}, task meanAP: {4:.3f},' \
                    'adv acc: {5:.3f}, n_processed: {6}'.format(batch_idx + 1,  task_loss_logger.avg, \
                    adv_loss_logger.avg, l1_loss_logger.avg, task_meanAP*100, accuracy_score(adv_truth, adv_preds)*100, nProcessed))
    ...
    print('Test epoch(f): {}, meanAP: {:.2f}, adv acc: {:.2f}, '.format(epoch, meanAP*100, adv_acc*100))

    return task_f1_score

def test(args, epoch, model, criterion, criterionL1, val_loader, val_logger, \
    print_every=1000000, logging=True):
    model.eval()
    nProcessed = 0
    task_preds, adv_preds = [], []
    task_truth, adv_truth = [], []
